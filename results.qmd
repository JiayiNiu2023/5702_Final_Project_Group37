...
# Question 4
What relationships exist between government debt levels and economic growth?
...
# Question 5
How do inflation and unemployment rates compare between developed and developing nations?
...
```{r}
inflation_long <- data %>%
  select(Status, starts_with("Inflation_")) %>%
  pivot_longer(
    cols = starts_with("Inflation_"),
    names_to = "Year",
    values_to = "Inflation_Rate"
  ) %>%
  mutate(Year = gsub("Inflation_", "", Year)) %>%
  na.omit()

mean_inflation <- inflation_long %>%
  group_by(Status, Year) %>%
  summarise(Mean_Inflation = mean(Inflation_Rate, na.rm = TRUE))

ggplot(mean_inflation, aes(x = Year, y = Mean_Inflation, color = Status, group = Status)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  theme_minimal() +
  labs(
    title = "Mean Inflation Rate Over Time",
    x = "Year",
