...
```
The boxplots compare unemployment rates between developed and developing nations from 2018 to 2023. Across all years, developed nations have lower median unemployment rates compared to developing nations. The interquartile range (IQR) for developed countries is narrower, indicating more stability in unemployment rates. In contrast, developing countries have greater variability, as exhibited by wider IQRs.
...
inflation_binned <- average_inflation_ %>%
  ...
  mutate(
    Proportion = Count / sum(Count)  # Calculate proportions within each group
  ) %>%
  ungroup() %>%
  na.omit()
...
```
From the graph, most developed and developing countries have average inflation rates in "0-5%" and "5-10%", but their distributions vary. Over 60% of developed countries fall within the "0-5%" range, while a smaller proportion of developed countries have inflation in the "5-10%" range, with few developed countries beyond 10% inflation rate. This pattern indicates the ability of developed countries to maintain low inflation rates consistently over time.

In contrast, while about 55% of developing countries also fall into the "0-5%" range, their inflation distribution is much more diverse. Unlike developed countries, there are a notable number of developing countries in the "10-15%" and "20%+" bins. This indicates that many developing countries face higher inflation pressures. In particular, the "20%+" bin highlights the challenges of hyperinflation in some developing economies.
